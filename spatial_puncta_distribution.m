function [spatial_distribution]=spatial_puncta_distribution(index, image_name, synaptic_log, compartment_log, dendritic_synaptic_log, dendritic_compartment_log, dendritic_puncta_syn,dendritic_puncta_com)
...
waitbar(i/max_total, f, sprintf('Social Distancing: %d %%', floor(100*i/max_total))); 
end
...
for j=1:size_den_syn
    if ismember(den_syn(j,4),dendritic_puncta_syn(:,1))
...
for i=1:n_compartment_den
    ...
    waitbar((i+n_synaptic_den)/max_total, f, sprintf('Social Distancing: %d %%', floor(100*(i+n_synaptic_den)/max_total)));  
...
for j=1:size_den_com
    if ismember(den_com(j,4),dendritic_puncta_com(:,1))
...
for i=1:n_synaptic
 ...
 waitbar((i+n_synaptic_den+n_compartment_den)/max_total, f, sprintf('Social Distancing: %d %%', floor(100*(i+n_synaptic_den+n_compartment_den)/max_total))); 
...
waitbar((i+n_synaptic_den+n_compartment_den+n_synaptic)/max_total, f, sprintf('Social Distancing: %d %%', floor(100*(i+n_synaptic_den+n_compartment_den+n_synaptic)/max_total)));
...
for i=1:size_den_syn
    ...
    x2=den_com(:,1:3);
    d=min(pdist2(x1,x2));
    spatial_distribution(counter,1)=d;
    counter=counter+1;
    
waitbar((i)/(size_den_syn+size_syn), f, sprintf('Social Distancing: %d %%', floor(100*(i)/(size_den_syn+size_syn))));
end
...
waitbar((i+size_den_syn)/(size_den_syn+size_syn), f, sprintf('Social Distancing: %d %%', floor(100*(i+size_den_syn)/(size_den_syn+size_syn))));
end
close(f);
end